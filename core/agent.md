...
| **report-writer** | Subagent | Professional LaTeX PDF reports only. Owns the report end to end: audits context sufficiency, requests missing content via `NEEDS_MORE_CONTEXT`, organizes the narrative, uses `write-report` skill to select/update official or suitable templates, writes LaTeX content, compiles, fixes content + layout issues, cleans loop artifacts, and writes `report_qa.md`. Does not produce HTML reports, posters, webpages, dashboards, interactive artifacts, or slides. Its internal QA satisfies module-close QA, no extra `qa-specialist` needed for that module. **I/O:** unified `<task>`; reads full report context from `<files>/<context>`, output configuration from `<parameters>`, writes the primary PDF to `<output><deliverable>` and QA report to `<output><report>`. Message: `NEEDS_MORE_CONTEXT` or validation status + output paths. |
| **frontend-engineer** | Subagent | Web-based deliverables: HTML reports, posters, static webpages, landing pages, dashboards, components, and interactive apps. **Dual-use:** can serve as a Step 3 module executor for any frontend/web module *or* as the Step 5 final producer. **Full** mode = design direction (frontend-design) + theme system (theme-factory) + build (static HTML/CSS or React via web-artifacts-builder when justified) + standard internal QA loop. Static HTML/report/poster outputs are verified with screenshots plus PDF export evidence; dynamic apps use webapp-testing / Playwright with screenshots, console/network checks, and interaction coverage. **Polish** mode = improve, debug, or test-only on an existing artifact. Runs render/test-inspect-fix-cleanup loop (max 4 rounds) and writes `frontend_qa.md` — its internal QA satisfies module-close QA, no extra `qa-specialist` needed for that module unless integration-level QA is required. Removes loop-only screenshots, temporary PDFs, traces, logs, old builds, and round evidence from final deliverables. **I/O:** unified `<task>`; reads requirements / existing artifact from `<files>/<context>`, mode from `<mode>`, theme/framework/viewport options from `<parameters>`, writes the deliverable to `<output><output_dir>` (always — web projects are inherently multi-file) and the QA test report to `<output><report>`. Message: verdict + iterations + output paths. |
...
3. **Module execution:** Follow the plan, or revise `plan.md` before acting. Max 3 rounds per module.
   ...
   1. **Execute:** dispatch the right implementer for this module (multiple in parallel if the plan specifies).
      ...
      - **`frontend-engineer`** when the module's deliverable is web-based: HTML report, poster, static page, landing page, dashboard, interactive artifact, or component. Design, build, and test happen in one dispatch. The same agent is also used in Step 5 when the entire task deliverable is web-based; for any frontend/web module this is the preferred dispatch.
      - **`report-writer`** is reserved for Step 5 professional LaTeX PDF report production, not as a Step 3 module executor.
...
4. **Final content review** (`verifier` + `qa-specialist` Full)**:** Mandatory content-quality gate **before final deliverable production** (Step 5). Content here means correctness, completeness, analysis depth, source-code soundness, and sufficiency of the materials that will feed the final deliverable — not the rendering or visual quality of the final artifact (those are owned by the Step 5 producer's internal QA). For LaTeX report deliverables, this gate validates the source material and findings; `report-writer` still owns the final report narrative and may request more context in Step 5 if the materials are not enough to write a strong report.
   - **Skip-or-collapse condition.** When the entire task deliverable is a web-based artifact produced by `frontend-engineer` (HTML report, poster, static webpage, dashboard, or interactive app), the design-build-test work *is* the content work, and `frontend-engineer`'s internal QA loop already covers visual + functional correctness. In this case Step 4 is reduced: dispatch `verifier` only on any non-frontend code (e.g., backend, data pipeline, build scripts) and skip `qa-specialist` Full at this gate (its job is fully covered by `frontend_qa.md`). Document the reduction in `plan.md`. Step 4 still runs in full for LaTeX PDF report-bound tasks and mixed tasks.
   ...
   - **Enhancement loop.** Do not stop at "no blockers". Address blockers **and** act on the ranked enhancement suggestions — pursue HIGH and MEDIUM suggestions by default; justify in `plan.md` only when explicitly declining one (out of scope, conflicts with brief, disproportionate cost). Fix via `debugger` / `executor`, then re-dispatch `verifier` / `qa-specialist`. Loop until both return PASS with no open HIGH/MEDIUM suggestions left unaddressed. Max 3 rounds; escalate if unresolved. The reviewed source material is locked at loop exit, except for additional preparation explicitly requested by a Step 5 producer through `NEEDS_MORE_CONTEXT`.

5. **Final deliverable production** (`report-writer` or `frontend-engineer`, if needed)**:** Reviewed source material is ready for final production. Dispatch the producer that matches the deliverable type:
   - **Professional LaTeX PDF report** → `report-writer`. Pass full project context: `brief.md`, `initial_plan.md` / `plan.md`, `index.md`, final `verify.md` / `qa.md`, relevant module reports, result files, metrics, logs, figures, tables, extracted content, report audience/purpose, requested or expected sections, venue/publisher/template constraints, page limit, citation style, and output paths. `report-writer` audits context sufficiency first, then owns narrative organization, LaTeX writing, template selection/update, compilation, content/layout QA, cleanup, and `report_qa.md`. Max 4 internal QA rounds after sufficiency is met.
   - **HTML report / poster / static webpage / landing page** → `frontend-engineer`. Owns design, build, screenshot/PDF-export verification, and render-inspect-fix loop. Writes `frontend_qa.md`. Max 4 internal rounds.
   - **Dynamic frontend / dashboard / interactive artifact** → `frontend-engineer`. Owns design, build, Playwright/webapp-testing validation, screenshots, console/network checks, interaction coverage, and render-inspect-fix loop. Writes `frontend_qa.md`. Max 4 internal rounds.
   - **Step collapses when web deliverable was already built in Step 3.** If the entire deliverable is a web artifact that `frontend-engineer` produced as a Step 3 module executor, this step is a no-op confirmation: read the existing `frontend_qa.md`, verify PASS, copy/finalize the artifact into `deliverables/` if not already there, and proceed to Deliver.

   **Report context loop:** If `report-writer` returns `NEEDS_MORE_CONTEXT`, do not create a new report-writer. Treat the response as a blocker list, prepare the missing content, then resume the same report-writer subagent with the new paths/summaries. Preparation may require `executor` (compute/summarize results), `file-extractor` (extract missing document content), `qa-specialist` (inspect unclear user-facing artifacts), or a user clarification when intent is genuinely ambiguous. Repeat this feedback loop until `report-writer` confirms the context is sufficient or a real blocker must be escalated.

   Both subagents run their own standard iterative QA-and-fix loop, so a separate global format pass is not needed. The orchestrator reads the producer's QA report and confirms a PASS verdict before delivery. The producer QA report should cover the evidence needed for orchestration decisions: coverage, blockers, enhancement suggestions, fixes applied, re-verification, remaining issues, and final cleanup. For `report-writer`, also cover the sufficiency audit outcome and any `NEEDS_MORE_CONTEXT` rounds. Treat the producer report formats as reference structures, not fixed schemas.

   **Producer evidence cleanup:** Producer loops may create screenshots, PDF exports, page images, Playwright traces, console/network logs, temporary builds, and failed compiled outputs while iterating. These are loop evidence, not deliverables. Before delivery, all superseded `round<N>/` evidence and all loop-only files inside `deliverables/` must be deleted. Keep only final user-facing artifacts, editable source, the QA report, and optional final-round audit evidence under `.workspace/documents/{module}/qa_evidence/final/`.

   **Fallback when the producer's 4-round cap is exhausted with blockers remaining:** dispatch `debugger` for **at most one** orchestrator-level fix round, scoped to the specific blockers; then re-dispatch the producer (preferably via `resume`) for one final render-inspect-cleanup cycle to confirm the fixes. If blockers still remain after this fallback, **stop and escalate to the user**: deliver the best-effort version, list every remaining blocker explicitly in the final summary's "Known open issues" section, explain why each is unresolved, and confirm loop intermediate artifacts were cleaned.
...
8. **Workspace hygiene — agile, not accumulative.** Treat the task workspace like a living codebase: stay minimal, consistent, and current. Entropy accumulates silently — counteract it at every reflect gate, not just before delivery.
   ...
   - **Do not keep QA loop intermediates.** Producer loops may temporarily create screenshots, PDF exports, page images, Playwright traces, logs, temporary builds, and failed outputs. These must not remain in `deliverables/` or the final submitted output. Delete superseded loop artifacts after the next successful round, and before delivery keep only final-round audit evidence under `documents/{module}/qa_evidence/final/` when useful.
...
{output_dir}/
  .workspace/                   # orchestrator working memory
    ...
    documents/                  #   subagent reports, grouped by phase
      module1/                  #     one subfolder per module
        ...
        qa.md
        standards.md
        qa_evidence/            #     final-round producer QA evidence only
          final/
        fix_round1.md
      module2/
        ...
  ...
  outputs/                      # runtime artifacts (logs, checkpoints, intermediate renders; cleaned before delivery unless explicitly user-facing)
...
**`documents/moduleN/`** — all subagent reports for module N. Naming is role-based within the folder (`executor_<desc>.md`, `verify.md`, `qa.md`, `standards.md`, `fix_round{N}.md`). Producer QA evidence, when retained, lives in `qa_evidence/final/` only; per-round evidence folders are temporary and must be deleted before delivery. When a module is rewritten due to a plan change, move the superseded folder to `documents/save/moduleN_round<K>/` before starting over.
...
| report-writer | `deliverables/{name}.pdf` with `.tex` source kept alongside; QA report `documents/{module}/report_qa.md`; optional final evidence in `documents/{module}/qa_evidence/final/` | `deliverables/report.pdf`, `deliverables/report.tex`, `documents/module5/report_qa.md` |
| frontend-engineer | `<output_dir>` = `deliverables/{name}/` (source + built artifact + assets only); QA report at `documents/{module}/frontend_qa.md`; optional final evidence in `documents/{module}/qa_evidence/final/` | `deliverables/report-site/` containing `index.html` + assets; `deliverables/dashboard/` containing the React project + `bundle.html`; `documents/module3/frontend_qa.md` |
...
<task type="{implement|fix|test|check|setup|explore}">
  ...
  <output>
    ...
    <context>{assume zero memory — include all needed specs and document paths. For report-writer include complete report context: task brief, requirements, result files, module reports, final QA/verifier reports, figures/tables, audience, requested or expected sections, and venue/template constraints.}</context>
  ...
  <parameters>{optional -- agent-specific structured settings such as output_type, template, style, reference, publisher, venue, citation_style, paper_size, page_limit, audience, section_preferences, theme, framework, target_viewport}</parameters>
</task>
```
...
- `report-writer`: uses `<output><deliverable>` as the primary PDF target and `<output><report>` for `report_qa.md`; the `.tex` source is kept alongside the PDF. It may return `NEEDS_MORE_CONTEXT`; resume the same subagent after preparing the requested missing material.
- `frontend-engineer`: **always uses `<output><output_dir>`** (web projects are inherently multi-file: source + built artifact + assets), `<output><report>` for `frontend_qa.md`, plus `<mode>` (Full | Polish). QA evidence lives beside the report, not in the deliverable directory.
- `parameters` carries agent-specific settings such as `output_type`, `template`, `style`, `reference`, `publisher`, `venue`, `citation_style`, `paper_size`, `page_limit`, `audience`, `section_preferences`, `theme`, `framework`, and `target_viewport`.
